# miscellaneous

# 雑多なプロジェクトとプログラム例集 / miscellaneous project and examples  
このリポジトリは、ブログに連動した雑多なサンプルプログラムをまとめたものです。  
This repository is a simple collection of miscellaneous sample programs linked with my blog posts.  

---

## 内容 / Contents
- `motor_speed_measurement/` :  
アナログ出力付きのモータ回転数測定 / Motor speed measurement with analog output  
使用したボード：Arduino NANO R4 / Applied board: Arduino NANO R4
Blog:  https://inte-gonext.hatenablog.com/entry/2026/----
- (preparing)  

---

## License
Copyright (c) 2026 inteGN - MIT License  

